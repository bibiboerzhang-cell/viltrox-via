...
export interface VkpiKolPoolVideoRow {
  ...
  /** 历史付费结果仍在库中，但未通过当前 canonical 复用证明。 */
  has_final_v1_raw_cache?: boolean;
  analysis_cache_reuse_status?: "canonical" | "legacy_unverified" | "stale" | string;
...
export interface VkpiMyKolVideoPage {
  ...
  summary?: { total?: number; views_total?: number; views_measured?: number; final_v1_ready?: number; legacy_unverified?: number };
  page?: VkpiVideoPageInfo;
  total?: number;
  returned?: number;
  has_more?: boolean;
...
export function classifyVideoRow(video: VkpiKolPoolVideoRow): VContentTier {
  if (video.revalidation_required || video.analysis_cache_reuse_status === "legacy_unverified") {
...
export function summarizeKolVideos(videos: VkpiKolPoolVideoRow[]) {
  ...
  return {
    ...
    unanalyzed: classified.filter(({ video }) => !video.has_final_v1_cache && !video.revalidation_required && !isImageKindVideo(video)).map(({ video }) => video),
  };
}
...
export function videoRecordRows(video: VkpiKolPoolVideoRow): Array<[string, string]> {
  return [
    ...
    ["深析缓存", video.revalidation_required ? "历史结果待核验(仅供查看)" : video.has_final_v1_cache ? "final_v1 ready" : "无(可入队深析)"],
    ["原帖", String(video.content_url || "—")],
  ];
}

/* ============ ④ 库行模型:aggregate.pool_favorites → KolLibraryRow ============
   实现在 ./myKolBoard-api.library(本文件触顶 1000 行硬卫兵后外迁),原样再导出。 */
export * from "./myKolBoard-api.library";
