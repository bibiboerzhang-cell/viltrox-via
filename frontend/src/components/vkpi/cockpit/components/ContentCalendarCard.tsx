  ...
  return e(m.div, {
    ...
    e("div", { className: "mb-3 flex items-center justify-between" },
      e("div", { className: "flex items-center gap-2" },
        ...
        e("h3", { className: "text-sm font-semibold text-ink" }, "近 7 天内容信号")
      ),
      onViewAll && days.length > 0 && e("button", { onClick: onViewAll, className: "text-[10px] text-muted hover:text-ink" }, t("View Full →"))
    ),
    e("div", { className: "space-y-1.5" },
      days.length === 0
        ? e("div", { className: "rounded-md border border-dashed border-line px-3 py-8 text-center text-[11px] text-muted" }, "暂无真实内容信号")
        : days.map((d: any, dayIndex: any) => e("div", {
        key: `${d.day || "day"}-${d.weekday || ""}-${dayIndex}`,
        className: `rounded-md px-2 py-1.5 ${d.today ? "bg-accent-soft border border-accent" : ""}`
      },
        e("div", { className: "flex items-center gap-2 mb-1" },
          e("span", { className: `text-[9px] font-semibold ${d.today ? "text-accent" : "text-muted"} w-8` }, d.weekday),
          e("span", { className: `text-[10px] ${d.today ? "text-accent" : "text-muted"}` }, d.day),
          d.today && e("span", { className: "text-[9px] text-accent ml-1" }, "TODAY"),
          e("span", { className: "ml-auto text-[9px] text-muted" }, `${d.items.length} 项`)
        ),
        d.items.length > 0 && e("div", { className: "space-y-1 pl-9" },
          d.items.slice(0, 3).map((item: any, i: any) => e("div", {
            ...
            className: "flex items-center gap-1.5 text-[10px] cursor-pointer hover:bg-accent-soft rounded px-1"
          },
            e("span", {
            ...
            e("span", { className: "text-muted shrink-0 w-9" }, item.time),
            e("span", { className: "text-ink-2 truncate flex-1" }, item.label)
          ))
        )
      ))
    ),
