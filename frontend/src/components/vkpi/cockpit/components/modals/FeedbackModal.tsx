...
import { submitCockpitFeedback } from "../../api";
...
export function FeedbackModal({ onClose, apiToken, onSubmitted }: any) {
  ...
  const submit = async () => {
    ...
    setStatus("");
    try {
      const result = await submitCockpitFeedback(apiToken, {
        category,
        title: title.trim(),
        description: description.trim(),
        screenshot,
        metadata: { source_page: "cockpit" },
      });
  ...
  return e(CenterModal, { onClose, maxWidth: "md" },
    ...
    e("div", { className: "p-5 space-y-3" },
      ...
      e("div", null,
        e("input", {
          id: "cockpit-feedback-screenshot-input",
          ref: fileInputRef,
          type: "file",
          accept: "image/*",
