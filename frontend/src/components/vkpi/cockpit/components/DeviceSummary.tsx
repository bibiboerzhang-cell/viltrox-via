...
export function DeviceSummary({ devices }: { devices?: any }) {
  if (!devices || !devices.camera_body) return <span className="text-slate-600 text-[10px]">—</span>;
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-[10px] text-slate-300 truncate max-w-[80px]">{devices.camera_body}</span>
      {hasViltrox && (
        <span
