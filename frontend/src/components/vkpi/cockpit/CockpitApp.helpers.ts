...
export function buildMappedEvents(eventRows: any) {
  const EVENT_TYPE_VISUAL: Record<string, { icon: any; color: string }> = {
  ...
  return (Array.isArray(eventRows) ? eventRows : []).map((row: any) => {
...
export function buildUpcomingEvents(mappedEvents: any) {
  const nowLocal = new Date();
  const todayStr = `${nowLocal.getFullYear()}-${String(nowLocal.getMonth() + 1).padStart(2, "0")}-${String(nowLocal.getDate()).padStart(2, "0")}`;
  return mappedEvents
    .filter((ev: any) => String(ev.startDate || "") >= todayStr)
    .sort((a: any, b: any) => String(a.startDate || "").localeCompare(String(b.startDate || "")))
    .slice(0, 6);
}

// 地图落点:每个「有定位」的活动一个 pin。
export function buildEventPins(mappedEvents: any) {
  return mappedEvents
    .map((ev: any) => {
...
// 计算当前要在地球上显示的 pins。
export function buildPins({ currentMode, isAvailable, country, city, item, venue, hierarchy, eventPins }: any) {
  if (!currentMode || !isAvailable) return [];

  // 真实活动上图:有定位的活动逐个落点(点击 → preview)。
  if (currentMode.isEvents) {
    return eventPins.filter((p: any) => {
  ...
  if (!country) {
    return Object.entries(hierarchy).map(([name, data]: [string, any]) => ({
  ...
  if (!city && hierarchy[country]?.cities) {
    return Object.entries(hierarchy[country].cities).map(([cityName, cityData]: [string, any]) => ({
      name: cityName,
  ...
  if (item && hierarchy[country]?.cities[city]) {
    const cityData = hierarchy[country].cities[city];
    const items = cityData.kols || cityData.stores || [];
    const sel = items.find((d: any) => (d.handle || d.name) === item);
    if (!sel) return [];
    ...
    if (venue && sel.venues) {
      const v = sel.venues.find((x: any) => x.name === venue);
...
// 计算 focusTarget。
export function buildFocusTarget({ country, city, item, venue, hierarchy, currentMode }: any) {
...
export function buildTopListData({ currentMode, country, city, item, hierarchy, viewMode }: any) {
  if (!currentMode) return { title: "", items: [] };
  if (!country) {
    const total = viewMode === "kols"
      ? Math.max(1, Object.values(hierarchy).reduce((sum: number, item: any) => sum + (Number(item.count) || 0), 0))
      : 100;
    return {
      title: viewMode === "kols" ? "Top Regions by KOL Count" : "Top Regions by Revenue",
      items: Object.entries(hierarchy)
        .slice()
        .sort(([, a]: [string, any], [, b]: [string, any]) => (b.count || parseFloat((b.revenue || "$0").replace(/[$k,M]/g, "")) || 0) - (a.count || parseFloat((a.revenue || "$0").replace(/[$k,M]/g, "")) || 0))
        .slice(0, 7)
  ...
  if (!city && hierarchy[country]?.cities) {
    return {
      ...
      items: Object.entries(hierarchy[country].cities)
        .slice()
        .sort(([, a]: [string, any], [, b]: [string, any]) => (b.count || parseFloat((b.revenue || "$0").replace(/[$k,]/g, "")) || 0) - (a.count || parseFloat((a.revenue || "$0").replace(/[$k,]/g, "")) || 0))
        .map(([name, d]: [string, any]) => ({
          label: name,
...
// Country options
export function buildCountryOptions({ currentMode, hierarchy, viewMode }: any) {
...
// City options
export function buildCityOptions({ country, hierarchy, viewMode }: any) {
...
export function buildItemOptions({ city, country, hierarchy }: any) {
...
export function buildVenueOptions({ item, city, country, hierarchy }: any) {
