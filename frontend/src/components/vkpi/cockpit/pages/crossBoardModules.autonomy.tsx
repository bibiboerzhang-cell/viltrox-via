...
const MIN_VERIFIED_SAMPLES = 20; // 已对答案样本少于此数 → 诚实空态「样本不足(n/20)」,不展示命中率
...
function ScorecardBody({ data }: { data: Row }) {
  ...
  const verified = Number(overall.in_range_judged || 0);
  return (
    <div>
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded-md border border-line p-2">
          ...
          <div className="mt-1 text-[18px] font-semibold text-ink">
            {verified < MIN_VERIFIED_SAMPLES
              ? <span className="text-[12px] font-medium text-muted">{t("样本不足")}({verified}/{MIN_VERIFIED_SAMPLES})</span>
              : fmtRate(overall.in_range_hit_rate as number | null)}
          </div>
        </div>
        <div className="rounded-md border border-line p-2">
