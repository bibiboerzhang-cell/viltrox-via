  ...
  return (
    <div className="mb-2">
      ...
      {filter.vOnly && vKolIds == null ? (
        <div className="mt-1.5 text-[9.5px] text-warn">V 名单未就绪(board-ext 未返回)——暂按「已挂项目」近似过滤,如实降级。</div>
      ) : null}
...
export function KolLibraryPageActions({
  ...
  return (
    <>
      {page ? (
        <div className="mt-2 flex items-center justify-between gap-2 rounded-[9px] border border-line px-3 py-2 text-[10px] text-muted">
          ...
          {page.has_more && page.next_cursor ? (
            <button
              ...
              className="rounded-[7px] border border-line-strong px-2.5 py-1 text-accent hover:border-accent disabled:cursor-wait disabled:opacity-50"
            >
      ...
      {loadMoreError ? <div className="mt-1 text-[10px] text-warn">{loadMoreError}</div> : null}
      <button
        type="button"
        onClick={onOpenList}
