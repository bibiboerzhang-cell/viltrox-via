...
import { ChevronRight, Moon, PanelLeftClose, PanelLeftOpen, Sun } from "lucide-react";
...
export function V615Sidebar({
  ...
  const [v2Open, setV2Open] = React.useState(() => NAV_ITEMS.some((i: any) => i.v2 && i.key === activeNav));

  // 单个导航按钮(主导航 + V2 组共用)
  const navButton = ({ icon: Icon, label, badge, key }: any) => {
  ...
  const visible = NAV_ITEMS.filter(({ key }) => perms.canViewBoard(key));
  const primaryItems = visible.filter((i: any) => !i.v2);
  const v2Items = visible.filter((i: any) => i.v2);
