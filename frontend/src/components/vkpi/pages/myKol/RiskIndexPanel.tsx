...
// 风险分 -> 色档(诚实分级,非评分指纹;纯展示)。
function riskColor(value: number): string {
  if (value >= 60) return '#f87171'; // 高风险:红
  if (value >= 35) return '#fbbf24'; // 中风险:黄
  return '#34d399'; // 低风险:绿
}
...
const CARD_BG = 'rgba(15,23,42,0.35)';
...
function DimensionRow({ dim }: { dim: RiskDimension }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11.5 }}>
      ...
      <div style={{ flex: 1, height: 6, borderRadius: 4, background: 'rgba(148,163,184,0.15)', overflow: 'hidden' }}>
        {!unavailable ? (
          <div
            style={{
      ...
      <span style={{ width: 56, textAlign: 'right', flexShrink: 0 }}>
        {unavailable ? (
          <span style={{ opacity: 0.45 }} title="该维度此 KOL 视频无对应深析分">未分析</span>
        ) : (
          <span>{pct(dim.score)}</span>
        )}
      </span>
      {dim.direction_confirmed === false ? (
        <span style={{ opacity: 0.5, fontSize: 10 }} title="该维度语义方向尚未经线上抽样确认">?</span>
      ) : null}
      {brands.length > 0 ? (
        <span style={{ fontSize: 10, color: '#f87171' }} title={`命中竞品:${brands.join(', ')}`}>
...
export function RiskIndexPanel({ apiToken, staffId }: RiskIndexPanelProps) {
  ...
  return (
    <section style={{ marginTop: 24, padding: 20, borderRadius: 14, border: BORDER, background: CARD_BG }}>
      <header
        style={{
          display: 'flex',
          alignItems: 'center',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 11.5, color: MUTED }}>
            ...
            {summary?.high_risk_count ? (
              <span style={{ color: '#f87171', marginLeft: 8 }}>· 高风险 {summary.high_risk_count}</span>
            ) : null}
          </span>
          <button
            type="button"
      ...
      {error ? (
        <p style={{ fontSize: 13, color: '#f87171', margin: '8px 0', display: 'flex', alignItems: 'center', gap: 6 }}>
          <AlertTriangle size={14} /> 读取失败:{error}
        </p>
      ) : null}

      {loading && items.length === 0 ? (
        <p style={{ fontSize: 13, opacity: 0.6, margin: '8px 0' }}>加载中…</p>
      ) : null}

      {!loading && items.length === 0 && !error ? (
        <p style={{ fontSize: 13, opacity: 0.6, margin: '8px 0' }}>该负责人名下暂无 KOL(收藏 / 共享均为空)。</p>
      ) : null}
      ...
      <p style={{ fontSize: 11, opacity: 0.5, marginTop: 14, lineHeight: 1.6 }}>
