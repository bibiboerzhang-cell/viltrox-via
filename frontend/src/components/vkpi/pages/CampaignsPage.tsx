  ...
  return (
    <PageShell title="活动 / 预算池 / 离职交接" description="P5 只落地 3 个：Campaigns、Budget Pool、Offboarding；成本状态适配 actual/void。">
      <section className="vkpi-card-grid vkpi-card-grid--forms">
        ...
        <section className="vkpi-card vkpi-action-card"><CardHeader title="离职 / 转岗交接" /><div className="vkpi-form-stack"><select value={offboardStaffId} onChange={(event) => setOffboardStaffId(event.target.value)}><option value="">选择离职成员</option>{data.staffMembers.map((member) => <option key={member.id} value={member.id}>{member.name} · {member.email}</option>)}</select><select value={newOwnerStaffId} onChange={(event) => setNewOwnerStaffId(event.target.value)}><option value="">新负责人（可选）</option>{data.staffMembers.map((member) => <option key={member.id} value={member.id}>{member.name} · {member.email}</option>)}</select><button className="vkpi-button vkpi-button--primary" disabled={busy || !apiToken || !offboardStaffId} type="button" onClick={() => void submitOffboarding()}>生成交接预案</button></div></section>
      </section>
