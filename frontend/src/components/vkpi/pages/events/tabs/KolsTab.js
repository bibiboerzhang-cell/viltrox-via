...
export default function KolsTab({ ev, token, invites = [], loading, error, reload }) {
  ...
  const [kolMap, setKolMap] = useState({});       // 真 KOL 池 id -> {name,handle,...}
  const [linkOpen, setLinkOpen] = useState(null);  // 展开追踪链的 invite id
  ...
  useEffect(() => {
    ...
    listMarketingKols(token, { limit: 300 })
      .then((res) => {
        if (cancelled) return;
        const map = {};
        (Array.isArray(res && res.kols) ? res.kols : []).forEach((r) => {
          const k = normKol(r);
          if (k.id) map[k.id] = k;
        });
        setKolMap(map);
