...
import AddProductPrepModal from "../modals/AddProductPrepModal";
...
export default function ProductPrepPanel({ ev, staff = [], stock, token, products: productsProp = [], loading, error, reload }: ProductPrepPanelProps) {
  ...
  const msg = (err: any) => String(err && err.message ? err.message : err);
  ...
  function renderRow(p: ProductVm) {
    const cat = PRODUCT_CATEGORIES[p.category] || { label: p.category || "其他", icon: Package, color: "#94a3b8" };
    const CI = cat.icon;
    const src = PRODUCT_SOURCES[p.source] || { label: p.source, color: "#94a3b8" };
  ...
  return e("div", { className: "p-5" },
    ...
    addingSide && e(AddProductPrepModal, {
      ...
      onSubmit: (p: ProductVm) => {
        ...
        addEventProduct(token, ev.id, p as unknown as Record<string, unknown>)
          .then(() => reload && reload())
