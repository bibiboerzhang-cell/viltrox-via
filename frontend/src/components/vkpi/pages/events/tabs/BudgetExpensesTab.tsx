...
export default function BudgetExpensesTab({ ev, currentUser, staff = [], token, expenses: rawExpenses = [], loading, error, reload }: BudgetExpensesTabProps) {
  ...
  function addExpense(data: ExpenseFormSubmit) {
    setShowModal(false);
            ...
            return e("div", { key: k, className: "px-3 py-2.5 border-b border-white/[0.04] last:border-0" },
              e("div", { className: "flex items-center justify-between mb-1.5" },
                ...
                e("div", { className: "text-[10.5px] tabular-nums" },
                  e("span", { className: overBudget ? "text-red-300" : "text-white" }, fmtMoneyShort(spent)),
                  e("span", { className: "text-slate-500" }, " / " + fmtMoneyShort(plan)),
                  overBudget && e("span", { className: "text-red-400 ml-1" }, "(超 ", fmtMoneyShort(spent - plan), ")"),
                  plan === 0 && e("span", { className: "text-amber-400 ml-1" }, "未预算")
                )
              ),
              ...
              e("div", { className: "flex items-center justify-between text-[9.5px] mt-1" },
                ...
                spent === 0 && plan > 0 && e("span", { className: "text-slate-500" }, "待支出")
              )
            );
          })
        ),
          ...
          e("div", { className: "flex items-center gap-2" },
            ...
            isOwner && e("div", { className: "flex items-center gap-0.5 rounded-md border border-white/[0.06] p-0.5" },
              ([
                ...
                ["all",  "全部 (负责人视角)", "#a855f7"],
              ] as Array<[string, string, string | undefined]>).map(([k, l, c]) => e("button", {
