...
interface DeletingTask { type: string; id: string; title: string }
...
export default function TasksTab({ ev, currentUser, token, tasks = [], loading, error, reload }: TasksTabProps) {
  ...
  const [addCollabFor, setAddCollabFor] = useState<string | null>(null);
  const [showNew, setShowNew] = useState(false);
  const [showAi, setShowAi] = useState(false);
  const [deleting, setDeleting] = useState<DeletingTask | null>(null);
  ...
  function toggleChecklistItem(taskId: string, itemIdx: number) {
              ...
              return e("div", { key: t.id, className: "hover:bg-white/[0.012] group" },
                ...
                e("div", { className: "px-3 py-2 flex items-center gap-2.5" },
                  ...
                  e("div", {
                    ...
                    e("div", { className: "flex items-center gap-2 flex-wrap" },
                      ...
                      t.kind && (() => {
                        const kc = TASK_KINDS[t.kind as string];
                  ...
                  // delete 按钮 (hover 出现)
                  e("button", {
                    onClick: (ev2: React.MouseEvent) => { ev2.stopPropagation(); setDeleting({ type: "task", id: t.id, title: t.title }); },
                ...
                expanded && hasDetail && e("div", { className: "px-3 pb-3 pl-10 space-y-2" },
                  ...
                  t.kind === "equipment" && t.details?.items && e("div", { className: "rounded border border-white/[0.04] bg-white/[0.012] overflow-hidden" },
                    ...
                    e("div", { className: "divide-y divide-white/[0.03]" },
                      t.details.items.map((item: DetailItem, i: number) => {
