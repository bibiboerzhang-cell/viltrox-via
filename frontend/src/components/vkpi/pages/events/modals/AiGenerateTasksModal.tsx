...
import { AI_TASK_TEMPLATES } from "../data/ai-templates";
import type { AiTaskTemplate } from "../data/ai-templates";
import { EVENT_TYPES, PHASE_LABELS, TASK_KINDS } from "../shared/constants";
import type { EventVm, TaskVm } from "../shared/types";
...
interface AiGenerateTasksModalProps {
  ...
  onClose: () => void;
  onSubmit: (tasks: TaskVm[]) => void;
}

export default function AiGenerateTasksModal({ ev, existingTitles, onClose, onSubmit }: AiGenerateTasksModalProps) {
  ...
  function submit() {
    const tasks: TaskVm[] = Array.from(selected).map(i => {
      const t = template[i];
      return {
  ...
  return e("div", { className: "fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4", onClick: onClose },
    e("div", { className: "rounded-2xl border border-purple-500/30 bg-[#0b1220] w-full max-w-2xl p-5 max-h-[92vh] flex flex-col", onClick: (ev2: React.MouseEvent) => ev2.stopPropagation() },
        ...
        : e(React.Fragment, null,
            ...
            e("div", { className: "flex-1 overflow-y-auto space-y-1.5 -mx-1 px-1" },
              Object.keys(PHASE_LABELS).map(phaseKey => {
                const phaseTasks = template
                  .map((t, i): AiTaskTemplate & { _idx: number } => ({ ...t, _idx: i }))
