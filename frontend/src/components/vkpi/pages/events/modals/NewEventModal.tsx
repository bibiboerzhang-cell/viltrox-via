...
import type { EventVm, ProjectVm, UiStaff } from "../shared/types";

const e = React.createElement;

export interface NewEventSubmit {
  title: string;
  typeKey: string;
  startDate: string;
  endDate: string;
...
interface NewEventModalProps {
  initialData?: EventVm | null;
  onClose: () => void;
  onSubmit: (data: NewEventSubmit) => void;
  teamOptions?: UiStaff[];
  currentUserId?: string | number;
  projects?: Array<Record<string, any>>;
}

export default function NewEventModal({ initialData, onClose, onSubmit, teamOptions = [], currentUserId, projects = [] }: NewEventModalProps) {
  ...
  const [teamIds, setTeamIds] = useState<string[]>(initialData?.teamUserIds || (currentUserId ? [String(currentUserId)] : (team[0] ? [String(team[0].id)] : [])));
  const [projectIds, setProjectIds] = useState<string[]>(initialData?.relatedProjectIds || []);
  ...
  const projectOptions: ProjectVm[] = ((Array.isArray(projects) && projects.length) ? projects : PROJECTS).map((p: Record<string, any>) => ({
    id: String(p.id ?? p.project_uid ?? ""),
    title: String(p.project_name || p.title || p.name || (p.id ?? "")),
  }));

  const toggleTeam = (id: string) => setTeamIds(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);
  const toggleProject = (id: string) => setProjectIds(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);

  return e("div", {
    className: "fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4",
    onClick: onClose
  },
    e("div", { className: "rounded-2xl border border-white/[0.08] bg-[#0b1220] w-full max-w-2xl max-h-[92vh] overflow-y-auto p-5", onClick: (ev: React.MouseEvent) => ev.stopPropagation() },
      ...
      e("div", { className: "space-y-3" },
        ...
        e("div", null,
          ...
          e("input", {
            type: "text", value: title, onChange: (ev: React.ChangeEvent<HTMLInputElement>) => setTitle(ev.target.value),
        ...
        e("div", null,
          ...
          e("input", {
            type: "text", value: productName, onChange: (ev: React.ChangeEvent<HTMLInputElement>) => setProductName(ev.target.value),
        ...
        e("div", { className: "grid grid-cols-2 gap-3" },
          e("div", null,
            ...
            e("input", { type: "date", value: startDate, onChange: (ev: React.ChangeEvent<HTMLInputElement>) => setStartDate(ev.target.value),
          ...
          e("div", null,
            ...
            e("input", { type: "date", value: endDate, onChange: (ev: React.ChangeEvent<HTMLInputElement>) => setEndDate(ev.target.value),
              className: "w-full px-3 py-2 rounded-md bg-white/[0.02] border border-white/[0.06] text-[11px] text-white focus:outline-none focus:border-purple-500/40 [color-scheme:dark]" })
          )
        ),
        ...
        e("div", null,
          ...
          e("div", { className: "space-y-2" },
            e("input", { type: "text", value: locName, onChange: (ev: React.ChangeEvent<HTMLInputElement>) => setLocName(ev.target.value),
            ...
            e("div", { className: "grid grid-cols-2 gap-2" },
              e("input", { type: "text", value: city, onChange: (ev: React.ChangeEvent<HTMLInputElement>) => setCity(ev.target.value), placeholder: "城市",
              ...
              e("input", { type: "text", value: country, onChange: (ev: React.ChangeEvent<HTMLInputElement>) => setCountry(ev.target.value), placeholder: "国家代码 (US / DE / JP)",
                className: "px-3 py-2 rounded-md bg-white/[0.02] border border-white/[0.06] text-[11px] text-white placeholder-slate-600 focus:outline-none focus:border-purple-500/40" })
            )
          )
        ),
        ...
        e("div", null,
          ...
          e("div", { className: "flex items-center gap-2" },
            e("div", { className: "relative flex-1" },
              ...
              e("input", { type: "number", value: budget, onChange: (ev: React.ChangeEvent<HTMLInputElement>) => setBudget(parseInt(ev.target.value)||0),
            ...
            e("label", { className: "flex items-center gap-1.5 text-[10.5px] text-slate-300 cursor-pointer" },
              e("input", { type: "checkbox", checked: autoCategories, onChange: (ev: React.ChangeEvent<HTMLInputElement>) => setAutoCategories(ev.target.checked), className: "accent-purple-500" }),
        ...
        e("div", null,
          ...
          e("div", { className: "flex flex-wrap gap-1.5" },
            team.map(u => {
              const active = teamIds.includes(String(u.id));
              return e("button", {
                key: u.id, onClick: () => toggleTeam(String(u.id)),
        ...
        e("div", null,
          ...
          e("textarea", { value: note, onChange: (ev: React.ChangeEvent<HTMLTextAreaElement>) => setNote(ev.target.value),
      ...
      e("div", { className: "flex items-center justify-end gap-2 mt-5 pt-4 border-t border-white/[0.05]" },
        ...
        e("button", {
          ...
          onClick: () => onSubmit({ title, typeKey, startDate, endDate, locName, city, country, budget, teamIds, ownerId: teamIds[0] || (currentUserId ? String(currentUserId) : (team[0] ? String(team[0].id) : "")), projectIds, productName, note }),
