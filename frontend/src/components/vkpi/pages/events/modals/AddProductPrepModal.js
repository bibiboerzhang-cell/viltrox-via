...
import { PRODUCT_CATEGORIES, PRODUCT_SOURCES } from "../shared/constants.js";

const e = React.createElement;
export default function AddProductPrepModal({ side, stock = [], onClose, onSubmit }) {
  ...
  const validSources = Object.entries(PRODUCT_SOURCES).filter(([k, v]) => v.side === side);
    ...
    e("div", { className: "rounded-2xl border border-white/[0.08] bg-[#0b1220] w-full max-w-md p-5", onClick: ev => ev.stopPropagation() },
      ...
      side === "have" && stock.length > 0 && e("div", { className: "mb-3 flex items-center gap-0.5 rounded-md border border-white/[0.06] p-0.5" },
        [
          ["from_stock", "📦 从库存表选 (推荐)"],
          ["manual",     "✏️ 手动输入"],
        ].map(([k, l]) => e("button", { key: k, onClick: () => { setMode(k); setSelectedStockId(null); },
      ...
      side === "have" && mode === "from_stock" && e("div", { className: "space-y-3" },
        e("div", { className: "relative" },
          ...
          e("input", { type: "text", value: stockSearch, onChange: ev => setStockSearch(ev.target.value), placeholder: "搜索 SKU / 名称",
        ...
        selectedStockId && e("div", { className: "grid grid-cols-2 gap-3" },
          e("div", null,
            ...
            e("input", { type: "number", value: qty, min: 1, onChange: ev => setQty(parseInt(ev.target.value)||1),
        ...
        selectedStockId && e("label", { className: "flex items-center gap-2 text-[10.5px] text-slate-300 cursor-pointer" },
          e("input", { type: "checkbox", checked: returnAfter, onChange: ev => setReturnAfter(ev.target.checked), className: "accent-emerald-500" }),
          "展会后需要退还 (样品 / 借用 默认勾上)"
        )
      ),
      ...
      (mode === "manual" || side === "need") && e("div", { className: "space-y-3" },
        ...
        side === "need" && e("div", null,
          ...
          e("input", { type: "date", value: arriveBy, onChange: ev => setArriveBy(ev.target.value),
        ...
        e("label", { className: "flex items-center gap-2 text-[10.5px] text-slate-300 cursor-pointer" },
          e("input", { type: "checkbox", checked: returnAfter, onChange: ev => setReturnAfter(ev.target.checked), className: "accent-purple-500" }),
          "展会后需要退还 (样品 / 租赁 / 借用)"
        )
      ),
