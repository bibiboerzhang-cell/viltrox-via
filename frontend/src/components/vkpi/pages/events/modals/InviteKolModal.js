...
export default function InviteKolModal({ token, existingKolIds, onClose, onSubmit }) {
  ...
  useEffect(() => {
    ...
    listMarketingKols(token, { limit: 300 })
      .then((res) => {
        ...
        setPool(rows.map(normKol).filter((k) => k.id));
      })
      .catch((e2) => { if (!cancelled) setErr(e2 && e2.message ? String(e2.message) : "加载 KOL 池失败"); })
