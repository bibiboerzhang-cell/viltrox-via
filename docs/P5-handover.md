...
| 2026-06-12 20:3x-21:0x | **apply 链全程(波次1)** | 107/108/109/110 四迁移(psql 逐执+幂等复验+schema_migrations 记录;108 init=当日00:00 UTC 裁决版,453 行) | — | HUP→64991/64992 | 快照 viltrox2-pre107to110-20260611T203516.dump(50MB);C2 commit `5c492f15`(3 单测过);**PV-3 先遮后开**:遮 14 个 smoke/测试项目→scope 反转 `a54b60c6`→HUP 六件激活(C2/排序/d8/管2/管3/PV-3);employee 可见 39/遮 14;**C5 执行 769 对/713 KOL**(=777−8,口子#2 修复后 dp 行被同一函数追加命中,函数口径自动收紧;污染零混入,幂等复跑 0);C4 最小件(My KOL「我的收藏(Pool)」分区,随 F 件候"无恙"commit);**统一重 build:dist=a54b60c6=HEAD,漂移修复**;指纹全程 1123/32828.7260/1123 纹丝不变 |
| 2026-06-12 21:xx | **第五起合法漂移对账**(入口诊断裁决⓪) | 111(real_er 影子,已 apply) | — | — | **行 1123→1128(+5),SUM(fit_score) 32828.7260 守恒不变**。归因:0号 RealER 重抓批次 11:54–11:57 落库 5 个 TikTok KOL(id 5663–5667,`source_type=manual`/`created_by_staff_id=NULL`=脚本插入/`real_er n=10 method=evidence10_pooled_v1`)。卫生闸:5 行 `viltrox_fit_score=NULL`(0 行带分、0 行 `rule_v0_backfill` 留痕命中)→ 对 SUM 零贡献,评分集冻结未破;`viltrox_fit_reason=''` 空串使 `COUNT(reason)` 1123→1128(非真分值理由,纯口径噪声)。分母变化出处清楚、评分总额守恒——对账过 |
| 2026-06-12 22:4x | **第六起合法漂移对账**(架构审计窗口) | — | — | — | **行 1128→1134(+6),SUM 32828.7260 守恒、scored 999 不变**。出处:**架构审计纯只读零写**;漂移是**探针5 命门那条废数据 `smart_search_profile_advance`(job 944)级联仍异步后台运行**,持续发现新 TikTok/YouTube 创作者插库(id 5668-5673,`source_type=manual`/`created_by_staff_id=NULL`/`fit=NULL`→对 SUM 零贡献)。这是「护栏 record-only、无界级联」P0 的活体演示。处置:试图掐排队级联被 auto-mode 正确拦截(越应用路径写共享队列),未绕过;候 Jianbo 定夺清理 |
| 2026-06-13 | **第七起对账(级联收尾)**(收尾执行令阶段一) | — | — | — | **无新数值漂移**:探针5 级联已全 drain(job944+ done29/failed3,无排队),未再新增池行,指纹守恒 **1134/32828.7260/1134**。处置(用户明令授权):id 5668-5673 `source_type` 由 manual 改 **test_probe**(留作图C-3 护栏失效活体证据、可识别,不删);metadata-only,不动 count/sum/reason。「掐排队」已 moot(队列先于执行自然清空) |
| 2026-06-13 | **第八起对账(用户真搜索)** | — | — | — | **行 1134→1137(+3),SUM 32828.7260 守恒、scored 999 不变**。出处:用户真实文字搜索 session 64「给我找一下适合我们新的 Z1pro 闪光灯的用户」→ profile-advance 管线 new_creator 发现并落库 3 个新 KOL(含 5674 tiktok:quintonbobbitt / 5675 momosukar,`source_type=manual`/`fit=NULL`→对 SUM 零贡献)。**合法业务漂移**(真查询真发现,非废数据)。注:护栏未 enforce 时此搜索无闸烧 Apify/Gemini;发现路径有 BUG(YouTube 把整句 query 当 handle,profile_discovery.py:808/:89-90),记 Phase 2 修 |
| 2026-06-13 02:5x | **第九起对账(ABCD 同步迸发收尾)** | — | worker 92498→**5415(.venv 规范启动)** | HUP 46951→6704/6705 | **行 1137→1142(+5,新发现 TikTok 行 5676-5681 全 `source_type=manual`/`fit=NULL`→对 SUM 零贡献;net+5 含一处历史删行记账差),SUM 32828.7260 守恒、scored 999 不变**。ABCD 四 commit(`0055199b` C 详情解耦+账户块 / `aecb2348` D 召回排中文 / `ced5d66e` B 视频R2+回灌闸B / `5643afd9` 前端信息卡+播放器+排中文勾选)**零写 viltrox_fit_score**(pool.py:892 未动)。**关键运维修正**:旧 worker 92498 系裸 homebrew py3.14 手工启动(**无 boto3→R2 静默降级**,正是 MEMORY 警告),已换 `scripts/start_worker.sh`(.venv,boto3 1.42.89,AI/apify 全 ready);admin 46951 经 lsof 证实**本就跑 .venv**(加载 .venv site-packages),HUP 后新代码上线、B 同步视频 R2 路径可用。前端 tsc 0 err + build 通过(dist gitignore,运行时直读)。队列 0 queued/0 processing 已抽干 |
| 2026-06-13 13:xx-14:0x | **第十起对账(P0 全量收尾,5/6 上线)** | 113 假粉/114 触达/115 contact/116 PII台账/117 DSAR | worker→74422(.venv) | HUP 46951→74450/74451 | **指纹 1142/32828.7260/999 全程守恒**。P0 六件:✅P0-6 地区口径{CN/HK/TW}(移除汉字误杀,挽回19海外号)`99c33554` / ✅P0-5 导出三类(全池/收藏/项目)`ea7f5fcb` / ✅P0-3 假粉规则离群独立flag(扫1142命中111,迁移113)`749c2aeb` / ✅P0-4 去重激活+profile_basics ON CONFLICT修复+触达回流(迁移114,email自动合并待P0-1邮箱激活,当前0候选)`ad2d9956` / ✅P0-1 邮箱+全球最严合规骨架(迁移115/116/117,feature_flag **默认OFF** 不抓不暴露)`9a03edda` / ⏸️P0-2 产品线维度(原稿前端补丁是描述性非精确码,需重设计,延后 task#25)。**红线全守**:5 迁移+去重+假粉flag 全程零写 viltrox_fit_score,SUM 守恒。**迁移撞号修正**:P0-4 与 P0-1 并行设计都选114,落地时 P0-1 改 115/116/117。**P0-1 二期未竟(flag开启前必补)**:_budget_ok 占位恒True(Apify预算闸未实接)、DSAR Qdrant/R2 delete TODO、读端email脱敏未落——故 email **尚未真抓取**,P0-4 auto-merge/外联依赖此 |
