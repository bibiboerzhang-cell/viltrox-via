...
## G 节原文合龙(2026-06-12 随令贴入,逐字)
 N1 沟通历史CRM:五月规格复活+报价字段,与G3同族,Pool切面先行
 N2 「刷新此人」:Drawer单行即时enrich,限流+成本入账,实时的诚实形态①
 N3 「本周异动」条:脉冲diff层,涨粉Top5/ER异动/掉粉警报,实时的诚实形态②,My KOL提醒地基
 N4 批量操作:多选行→批量收藏/入队深析/标签
 N5 保存视图:每员工存筛选组合
 N6 对比视图:勾2-4人并排比(fit/四指标/雷达/CPM)
 N7 SSE替代泳道轮询:排末可不做
- **N1=生(裁决)**:骨架 spec → docs/N1-outreach-spec-recovered.md(标"骨架版·细节以原文为准";报价条款 quote_amount+currency+quote_date 喂智能层 3 号 CPM 锚;四借力入依赖节)

## 程序法(五条合编成文,2026-06-12;序号以本档为准,如与口径不符报裁改)
1. **红线修订程序**:红线只许"意图不变、字面升级",须裁决并落档(排序条/fit写点条两例在案)
2. **tsc 红禁 commit**:体检不过不许落刀;amend 仅限未 push 紧急修正且回执自报(d4 首例在案)
3. **条件随字走**:放行字附带条件的,执行回执逐条对账,缺一不算完毕;多令叠发条件不被冲掉
4. **拦截波及同批**:守卫拦截后同批命令逐条标注已执行/未执行,恢复时逐条复核,不许假设
5. **送达制**:关键令以"收到+指定复述"为送达;复述(边界/条件/裁决)与对账表同级

## dist 漂移溯源(2026-06-12 记档)
- 现象:dist=8ff801a2(纲领件窗 build)早于最后已知 build f0c4aaa0(d7/d8 窗)——dist 出现回退,疑车道外 build 以旧 checkout 覆盖共享产物(车道二三未报到期间)。处置:下个车道一窗口统一重 build 对齐;commit 总线铁律重申:**build 只许车道一一只手**。

## 2026-06-12 apply 链收账(裁决五条)
- **① C5=769 定格**:数字服从执行时函数口径,770 作废;算术链终版入 dry-run 档头;第 8 对(3339 dp×assignment)入重建清单,**P6 重建清单现 8 对**
- **② C4 过渡态记账**:My KOL 顶部收藏分区 = **过渡态(透明申报)**;C4-full(EmployeeKolLibrary 主体改读收藏集)三依赖(107 表/C2 端点/769 真数据)已全活零阻塞——**下窗口首件,与 d11 同窗;过渡态不许溶解成终态**
- **③ 模式命名入册:「浏览器验收前置的合法领先」**——dist 可领先 HEAD 仅限 verify-then-commit 场景(改动已建未提交+已申报+验收字落即 commit 对齐);与未申报漂移(车道外 build)性质不同,后者仍为违规
- **④ 工程检查表新行(LIKE % 同坑二踩)**:**compat 层 SQL 凡含 LIKE/ILIKE 字面 `%`,一律参数化(`LIKE ?` + pattern 入参),禁止拼进 SQL 字符串**——psycopg 把 `%` 当占位符,两踩在案(补课a/PV-3 遮步)
- **⑤ 通告两行批准**,原文转发

## 2026-06-12 独占体制乙案 · 预研报告(与施工并行)
- **claims 现状**:vkpi_kol_claims 7 行(active 1),FK 挂 kols(id) 经 linked_main_kol_id join 池命中≈0——**不可用作独占数据源**;独占数据源定为**在役 assignment**(stage 非终态/流失 + 项目非 restricted)
- **最小施工面**(四件映射):①选择器=available 端点 SELECT 补 occupied_by(在役 owner 子查询)+前端置灰「已被 X 跟进」;②防绕过=add_project_kols 写前同口径校验;③存量归属=769 收藏已归 assignment 持有者(C5 归属规则同源);④释放=项目 stage 终态钩子自动释 + 手动释端点(留痕 audit)
- **⚠️ 存量冲突清单(核心待裁)**:多 owner 在役 KOL = **55 个**(同一 KOL 在不同项目由不同人跟进,如 1508: 7683+84 / 3670: 7686+7689…全单 12 样例在对话)——**乙案"一次一人"的维度需裁:A) 全局独占**(55 个冲突要逐个定主,跟进权收敛)**或 B) 项目维独占**(同项目一人=现状天然满足,选择器只灰"他人在役"并标示,跨项目共跟进合法)。**预研建议 B**:55 冲突即业务现实(同 KOL 多产品线项目),全局独占会强拆真实协作;B 案零存量手术,标示照样防撞车外联
