...
- `scripts/vkpi_apify_batch_refresh.py` is the dedicated operator CLI; it defaults to a blocked executor smoke, writes a JSON artifact under `runtime/ops`, includes top-level `operator_summary` and `provider_config`, and real Apify calls require both `--execute` and `--allow-provider-calls` plus configured provider credentials and the live target count staying within `--max-live-targets` (default 25, hard max 100).
