...
- `scripts/vkpi_apify_batch_refresh.py` is the dedicated operator CLI; it defaults to a blocked executor smoke, writes a JSON artifact under `runtime/ops`, includes a top-level `operator_summary`, and real Apify calls require both `--execute` and `--allow-provider-calls`.
