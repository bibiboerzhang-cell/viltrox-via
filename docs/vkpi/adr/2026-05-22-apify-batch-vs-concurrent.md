...
- `scripts/vkpi_apify_batch_refresh.py` is the dedicated operator CLI; it defaults to a blocked executor smoke, writes a JSON artifact under `runtime/ops`, includes top-level `operator_summary`, `provider_config`, `execution_preflight`, and `safe_live_windows`, and real Apify calls require both `--execute` and `--allow-provider-calls` plus configured provider credentials and the live target count staying within `--max-live-targets` (default 25, hard max 100).
- If the full qualified plan exceeds `--max-live-targets`, the CLI emits `window_execution_runbook.execute_commands` and supports `--live-window-index N` so operators can run one safe window at a time after explicit provider authorization.
