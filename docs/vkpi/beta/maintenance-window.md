...
3. **大改:W5(周日 22:15 UTC)**,迁移不可前向兼容、依赖大版本(H-08)、systemd 单元变更(H-15 / A1 W1)一律走这里;24h 前公告,须在实际切换前完成隔离演练和审批。**选择维护窗口不等于获得迁移放行**：当前 308–310 超出标准班车的已审阅策略，须先补齐专用发布方案及旧应用兼容性证据；不得仅增加环境声明、先手工执行 SQL 或启用被禁止的 clone 路径。范围与回退边界见 H-18。
...
6. 迁移优先前向兼容；`VKPI_FORWARD_COMPATIBLE_MIGRATIONS` 只是已审阅迁移的精确声明，不是放行开关。不可兼容的迁移须另行确认停写维护方案，明确提交点及恢复策略后再选择 W5；应用自动回退不撤销数据库变更，不能把 `_down.sql` 当成无损自动回滚。
