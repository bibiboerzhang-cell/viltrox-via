...
def _read_worker_default_timeout() -> int:
    env = os.environ.copy()
    env.pop("APIFY_WORKER_GEMINI_CALL_TIMEOUT_SEC", None)
    env["PYTHONPATH"] = f"{BACKEND_ROOT}{os.pathsep}{env.get('PYTHONPATH', '')}"
    result = subprocess.run(
        [
            sys.executable,
...
class ApifyJobsWorkerTimeoutConfigTests(unittest.TestCase):
    def test_gemini_timeout_default_is_long_video_safe(self) -> None:
        self.assertEqual(_read_worker_default_timeout(), 1200)
