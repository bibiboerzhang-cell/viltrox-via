...
def seeded_data():
    ...
    conn.execute(
        """INSERT INTO vkpi_projects
           ...
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        ("VKPI-TEST-001", "Test Project", 9001, 7001, 7001,
         "shipped", "active", "normal", "manual", "shipped", "{}", now, now),
    )
    project_row = conn.execute("SELECT id FROM vkpi_projects WHERE project_uid='VKPI-TEST-001'").fetchone()
    ...
    conn.execute(
        """INSERT INTO vkpi_sales_attributions
           ...
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        ("shopify", "test-order-1", project_id, 9001, 7001, 10000,
         500, "USD", "last_touch", "confirmed", now, now, "{}", now),
    )
    sales_row = conn.execute("SELECT id FROM vkpi_sales_attributions WHERE source_ref='test-order-1'").fetchone()
    ...
    conn.execute(
        """INSERT INTO vkpi_cost_ledger
           ...
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""",
        (project_id, 9001, 7001, "product_cost", 3000, "USD", "actual",
         now, "test-cost-1", "test product cost", "{}", now),
    )
    cost_row = conn.execute("SELECT id FROM vkpi_cost_ledger WHERE source_ref='test-cost-1'").fetchone()
    cost_id = int(cost_row["id"])

    conn.commit()
    yield {"project_id": project_id, "sales_id": sales_id, "cost_id": cost_id, "kol_id": 9001, "staff_id": 7001}
    ...
    conn.execute("DELETE FROM vkpi_sales_attributions WHERE id=?", (sales_id,))
    conn.execute("DELETE FROM vkpi_cost_ledger WHERE id=?", (cost_id,))
...
def test_generate_run_writes_header_and_values(seeded_data):
    result = metric_lineage.generate_run(period_days=365, trigger_source="dashboard")
...
def test_generate_run_derived_metrics_correct(seeded_data):
    ...
    # roi = 7000 / 3000 = 2.3333
    assert abs(float(roi["value_numeric"]) - (7000 / 3000)) < 0.001
...
def test_generate_run_subset_metrics_only(seeded_data):
    result = metric_lineage.generate_run(period_days=365, metrics=["gmv", "cost"])
    run_detail = metric_lineage.get_run(result["run_id"])
...
def test_drilldown_value_filter_by_project(seeded_data):
    ...
    matching = drilldown.drilldown_value(int(gmv_value["id"]), project_id=seeded_data["project_id"])
    assert matching["row_count"] == 1

    not_matching = drilldown.drilldown_value(int(gmv_value["id"]), project_id=999999)
...
def test_drilldown_latest_finds_most_recent(seeded_data):
    ...
    metric_lineage.generate_run(period_days=365, trigger_source="dashboard")
    second = metric_lineage.generate_run(period_days=365, trigger_source="dashboard")
...
def test_list_runs_filter_by_trigger(seeded_data):
    metric_lineage.generate_run(period_days=365, trigger_source="dashboard")
    metric_lineage.generate_run(period_days=365, trigger_source="weekly_report")
...
def test_source_contribution_percent_correct(seeded_data):
    ...
    result = metric_lineage.generate_run(period_days=365)
    run_detail = metric_lineage.get_run(result["run_id"])
    gmv_value = next(v for v in run_detail["values"] if v["metric_key"] == "gmv")
