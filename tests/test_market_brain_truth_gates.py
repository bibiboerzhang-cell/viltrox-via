...
def test_learning_readiness_requires_all_three_real_evidence_legs(monkeypatch):
    ...
    assert "o.actual_result" in conn.eval_sql
...
def test_prediction_rollup_keeps_raw_metrics_but_gates_claims():
    ...
    assert at_gate["claimable_metrics"]["wape"] == 0.1
...
def test_record_eval_rejects_link_to_unfinalized_outcome(monkeypatch):
    import app.db.connection as connection
    ...
    assert result["ok"] is False
    assert result["reason"] == "outcome_not_finalized"
...
def test_record_eval_rejects_finalized_outcome_without_observed_evidence(monkeypatch):
    import app.db.connection as connection
    ...
    result = prediction_ledger.record_eval("run-1", 100, outcome_id=7)
    assert result["ok"] is False
    assert result["reason"] == "outcome_missing_observed_evidence"
...
class _FinalizedObservedOutcomeConn:
    ...
    def execute(self, sql: str, params: tuple = ()):
        ...
        if "SELECT id FROM vkpi_prediction_evals" in sql:
            return _Cursor([])
...
def test_record_eval_requires_metric_binding_to_linked_outcome(monkeypatch):
    import app.db.connection as connection
    ...
    assert rejected["reason"] == "actual_evidence_binding_required"
    assert conn.commits == 0

    accepted = prediction_ledger.record_eval(
        "run-1",
        100,
        outcome_id=7,
        actual_json={
    ...
    assert "verified_against_outcome" in str(conn.insert_params[4])
            ...
            return _Cursor([
                {
                    ...
                    "actual_result": None,
                    "window_7d": {"status": "filled", "metrics": {"contacted": True}},
                    "window_14d": None,
                    "window_28d": None,
                    "created_at": NOW,
                    "decided_at": NOW,
