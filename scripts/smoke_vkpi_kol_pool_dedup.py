...
def main() -> None:
    ...
    out("[1/4] 同 platform + handle → 不增行 (Layer 1)")
    ...
    if after_first - pre != 1:
        ...
        out(f"   PASS: 第一次 +1,第二次不增 ({after_first}→{after_second})")
    
    # ── 场景 2: dedup_key 函数验证 ──
    out("[2/4] dedup_key 函数: 同 email 同 handle 应一致")
    ...
    if key_a != key_b:
        ...
        out(f"   PASS: dedup_key 正确区分 email")
    
    # ── 场景 3: handle 大小写 normalize ──
    out("[3/4] handle 大小写 → normalize 后视为同一行")
    ...
    if after_second_case != after_first_case:
        ...
        out(f"   PASS: normalize 后视为同一行")
    
    # ── 场景 4: 跨平台不去重 ──
    out("[4/4] 不同 platform + 同 handle → 视为不同行")
