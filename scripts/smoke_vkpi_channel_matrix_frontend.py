...
def main() -> None:
    ...
    assert "selectedPlatform" in channels_page and "filteredChannelsRows" in channels_page, "platform filter wiring missing"
    ...
    assert "nextPageToken" in youtube_crawler and "min(1000" in youtube_crawler, "YouTube full-baseline pagination missing"
    assert "min(1000" in instagram_crawler and "instagram-scraper" in instagram_crawler, "Instagram baseline must use the posts scraper for deeper playback totals"
    assert "min(300" in tiktok_crawler, "TikTok baseline limit must allow more than 50"
