...
def main():
    ...
    out(f"  ✓ {weekly_report_templates.PROMPT_VERSION}")
    
    out("[3] TEMPLATES has 6 keys...")
    expected_keys = {
        "layer1_universal",
        "layer2_leader",
        "layer2_kol_ops",
    ...
    if missing:
        ...
        out(f"  ✓ {len(actual_keys)} templates")
    
    out("[4] Each template has required fields...")
    ...
    out("[5] template_keys_for_role(leader) returns 2 templates...")
    ...
    out("  ✓ leader: 2 templates (no personal)")
    
    out("[6] template_keys_for_role(kol_ops) returns 3 templates...")
    ...
    out("  ✓ kol_ops: 3 templates")
    
    out("[7] template_keys_for_role(unknown) falls back...")
    ...
    out("  ✓ unknown role gets layer1 + layer3")
    
    out("[8] build_prompt renders correctly...")
    ...
    out("[9] title_for renders correctly...")
    ...
    out(f"  ✓ titles: '{t1[:40]}...' / '{t2[:40]}...'")
    
    out("[10] build_prompt unknown template raises...")
    try:
    ...
    out("  ✓ unknown raises ValueError")
    
    out("[11] All section_prompts mapped (no missing)...")
    ...
    out(f"  ✓ {len(section_keys_in_templates)} sections, all mapped")
    ...
    if failures:
        ...
        out("VKPI_WEEKLY_REPORTS_OFFLINE_SMOKE_OK")
