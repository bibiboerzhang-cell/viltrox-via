...
def main() -> int:
    ...
    out("== 侦察结果 ==")
    out(f"合同表可搬候选: {len(contract_rows)}  缺口: {contract_gaps}")
    out(f"账本兜底候选:   {len(ledger_rows)}  缺口: {ledger_gaps}")

    if not candidates:
        out()
        out("真实费用行 0 条 — 诚实数据缺口,不造假:")
        out("  vkpi_project_contracts 里没有「已确认 + 有费用 + 挂 KOL」的合同,")
        out("  vkpi_cost_ledger 里也没有可定位 KOL 的 cash_fee 实付行。")
        out("  出路:上传合同并人工确认费用字段,或在报价卡手动录入报价(add_rate);")
        out("  在此之前 estimate_rate 只能走 cpm_benchmark_v0,发射台预算段会持续显示 estimate_only 警示。")
    ...
    for cand in candidates:
        ...
        try:
            ...
            out(f"  + 已写入 rate id={created.get('id')}: {line}")
            seeded += 1
            seeded_kols.add(kol_id)
        except Exception as exc:  # noqa: BLE001 — 单行失败如实报,不掩盖不中断
            out(f"  ! 写入失败: {line} -> {str(exc)[:200]}")

    out()
    mode = "write" if args.write else "dry-run"
    out(f"== 统计({mode})== 候选 {len(candidates)} | 写入/计划 {seeded} | 幂等跳过 {skipped_existing}")

    if args.write and seeded_kols:
        out()
        out("== 灌种后 estimate_rate 验证(应从 cpm_benchmark_v0 切到 recorded_rates_median_v0)==")
        for kol_id in sorted(seeded_kols):
            est = rate_card.estimate_rate(kol_id, conn=conn)
            out(f"  kol={kol_id} method={est.get('method')} p50={est.get('estimated_usd_p50')} "
