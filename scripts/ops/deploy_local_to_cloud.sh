...
# ── A1 W1 quick win: dedicated scheduler unit, default OFF ─────────────────
# Only VKPI_DEPLOY_SEPARATE_SCHEDULER=1 (controller env) installs/enables
# deploy/systemd/vkpi-scheduler.service.  Any other value leaves production
# topology untouched: the scheduler keeps living inside ${SERVICE_NAME} and the
# advisory leader lock (migration 249) guarantees a single owner even while
# both candidates coexist.  The unit name is deliberately not
# viltrox-2.0-scheduler.service: that name sits in LEGACY_WRITER_UNITS and must
# stay inactive.  Switch-over/rollback steps: deploy/systemd/README.md.
SEPARATE_SCHEDULER_SERVICE="vkpi-scheduler.service"
SEPARATE_SCHEDULER_UNIT_RELATIVE="deploy/systemd/vkpi-scheduler.service"
