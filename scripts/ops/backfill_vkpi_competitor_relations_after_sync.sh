...
if [ "${post_sync_safe}" != "1" ]; then
  echo "Refusing competitor relation backfill: daily sync has no verified provider completion (orchestration=${orchestration_status}, scope=${completion_scope}, provider=${provider_completion})" >&2
  exit 2
fi
