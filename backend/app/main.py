...
from app.main_request_guards import (
    ...
    board_gate_allows as _board_gate_allows,
...
def _admin_rbac_allowed(request) -> bool:
    ...
    # 2026-07-18 权限双洞修:tab 检查后叠加 board.* 可见性闸(逻辑集中在 board_gate_allows)。
    return check_tab_permission(staff, permission_key, level) and _board_gate_allows(staff, path, request.query_params.get("board"))
...
# prod 落到 API 404。KOL 门户预授权页(login/student-signup)保持 public 闸。
@app.get("/login")
...
# 激活/重置是员工预授权页:prod 运行态 IS_PUBLIC_APP=false(实证 /activate 也 404),
# admin 面必须同样服务,否则成员激活与密码重置链接全断。
@app.get("/activate")
@app.get("/reset")
def read_staff_prelogin_surface():
    if not (IS_PUBLIC_APP or IS_ADMIN_APP):
        raise HTTPException(status_code=404, detail="Auth surface is disabled on this instance")
    return _serve_frontend()
