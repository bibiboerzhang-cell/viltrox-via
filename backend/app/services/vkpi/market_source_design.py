"""P5.66 market signal source design registry.

This module is intentionally read-only. It records source contracts and
readiness gates so later market intelligence work does not start by crawling
unbounded external sources.
"""
...
def _source_readiness(source: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        **source,
        ...
        "can_collect_now": can_collect_now,
...
def build_market_source_design_report() -> dict[str, Any]:
    ...
    tables = {
        table_name: {
            ...
            "rows": _count(table_name),
        }
        for table_name in table_names
    }
    sources = [_source_readiness(source) for source in SOURCE_REGISTRY]
