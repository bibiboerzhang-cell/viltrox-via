...
def _rule_v0_comment_summary(*, cutoff: str, sample_limit: int) -> dict[str, Any]:
    ...
    if cached == 0:
        status = "no_cached_comments"
    elif cached > safe_limit:
...
def overview(*, days: int = 7, recent_limit: int = 8) -> dict[str, Any]:
    ...
    rule_v0 = _rule_v0_comment_summary(cutoff=cutoff, sample_limit=safe_recent)
    return {
        "days": safe_days,
        "health": health,
        "provider_calls": False,
