"""P6.76 read-only 24h new signal digest.

This module turns existing trend, market, and comment records into an
actionable daily signal summary. It does not fetch providers, call LLMs, write
the database, enqueue tasks, or trigger sync.
"""
...
def build_today_new_signals_v0(
    *,
    ...
    trend_items = _recent_trend_items(trend_report, now=now, lookback_hours=bounded_hours, limit=bounded_limit)
    market_items = _recent_market_items(_market_rows(500), now=now, lookback_hours=bounded_hours, limit=bounded_limit)
    comment_summary = _recent_comment_summary(_comment_rows(5000), now=now, lookback_hours=bounded_hours, limit=bounded_limit)
    ...
    checks = {
        ...
        "source_pipeline_available": bool(trend_report.get("passed") or trend_items or market_items or comment_summary.get("contract", {}).get("cached", 0) > 0),
