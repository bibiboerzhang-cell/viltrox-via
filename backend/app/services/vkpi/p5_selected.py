...
def create_budget_pool(body: dict[str, Any], *, staff: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    get_conn().execute(
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?)
        """,
        (uid, name, body.get("period_start"), body.get("period_end"), total_cents, str(body.get("currency") or "USD"), _int(body.get("owner_staff_id"), _actor(staff)) or None, str(body.get("status") or "active"), _json(body.get("metadata") or {}), now, now),
    )
    get_conn().commit()
    row = get_conn().execute("SELECT * FROM vkpi_budget_pools WHERE pool_uid=?", (uid,)).fetchone()
    return {"budget_pool": dict(row) if row else {}}
...
def allocate_budget(pool_id: int, body: dict[str, Any], *, staff: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    get_conn().execute(
        ...
        VALUES (?,?,?,?,?,?,?,?)
        """,
        (int(pool_id), _int(body.get("campaign_id")) or None, _int(body.get("project_id")) or None, _int(body.get("staff_id")) or None, cents, str(body.get("note") or ""), _actor(staff) or None, _utcnow()),
    )
    get_conn().commit()
...
def execute_offboarding(run_id: int, *, staff: dict[str, Any] | None = None, actions: list[str] | None = None) -> dict[str, Any]:
    ...
    return {"offboarding": dict(row) if row else {}, "result": log}
