...
@router.get("/health")
async def deepsight_health(request: Request, staff=Depends(require_tab("deepsight", "read"))):
    return {
        "status": "ok",
        "modules": [
            "neural_brain",
...
async def build_pack(request_http: Request, body: DeepSightRequest, staff=Depends(require_tab("deepsight", "write"))):
...
@rate_limit("admin_mutation", max_requests=30, window_sec=300)
async def diagnose(request_http: Request, body: DeepSightRequest, staff=Depends(require_tab("deepsight", "write"))):
...
async def scan_official_matrix(request_http: Request, body: dict | None = None, staff=Depends(require_tab("deepsight", "write"))):
    body = body or {}
...
async def cache_stats(request_http: Request, staff=Depends(require_tab("deepsight", "read"))):
    return {"status": "success", "cache": cache.stats()}


@router.post("/cache/clear")
@rate_limit("admin_mutation", max_requests=20, window_sec=300)
async def cache_clear(request_http: Request, staff=Depends(require_tab("deepsight", "write"))):
    cleared = cache.clear()
