...
# 行为不变搬迁:项目物料库两端点(GET/POST /projects/{project_id}/materials)整组
# move 到 vkpi_projects_materials.py(子 router 无 prefix);此处原位 include_router 兜住,
# 路由顺序保留。Python 导入面按 analysis 簇同款 re-export。
from app.api.routers import vkpi_projects_materials as _materials_sub  # noqa: E402

router.include_router(_materials_sub.router)
list_project_materials = _materials_sub.list_project_materials
add_project_material = _materials_sub.add_project_material
