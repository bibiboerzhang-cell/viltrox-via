...
_EXACT_REASONS: dict[str, tuple[str, str]] = {
    ...
    "budget_scope_not_configured": ("budget", "这项功能还没有配额度,已暂停:请联系管理员"),
    "budget_registry_not_configured": ("budget", "这项功能还没有配额度,已暂停:请联系管理员"),
    "budget_row_invalid": ("budget", "额度设置有误,已暂停:请联系管理员核对"),
    "budget_single_call_cap_exceeded": ("budget", "这次请求太大,超过了单次上限:请缩小范围后重试"),
    "llm_spend_budget_not_configured": ("budget", "预算功能未开启:请联系管理员"),
    "provider_not_configured": ("model", "还没有接上可用的模型服务:请联系管理员完成配置"),
    "provider_calls_force_offline": ("model", "外部模型调用已被临时关闭:请联系管理员"),
    "provider_calls_blocked": ("model", "外部模型调用已被关闭:请联系管理员"),
    "no_provider_candidates": ("model", "暂时没有可用的模型"),
    "model_binding_blocked": ("model", "分析模型暂不可用:请稍后重试"),
