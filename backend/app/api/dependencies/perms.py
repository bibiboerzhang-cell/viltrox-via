...
def require_tab(tab_key: str, level: str = "read"):
    async def dep(request: Request, user=Depends(get_user_required)):
        staff = getattr(request.state, "vkpi_authorized_staff", None)
        if staff is None:
            staff = await asyncio.to_thread(staff_context_for_user, user)
...
def require_tab_stream(tab_key: str, level: str = "read"):
    """Apply normal tab RBAC after one-time SSE ticket authentication."""
    async def dep(request: Request, user=Depends(get_user_required_stream)):
        staff = getattr(request.state, "vkpi_authorized_staff", None)
        if staff is None:
            staff = await asyncio.to_thread(staff_context_for_user, user)
        if not check_tab_permission(staff, tab_key, level):
