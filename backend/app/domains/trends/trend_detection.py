"""Pure trend detection rule engine."""
...
def build_trend_detection_report(
    *,
    ...
    """Build a read-only trend detection report from already-loaded anchors."""
    ...
    signals = (
        _detect_post_trends(post_rows, lookback_days=bounded_lookback_days, now=resolved_now)
        + _detect_channel_trends(channel_rows, lookback_days=bounded_lookback_days, now=resolved_now)
        + _detect_market_signal_trends(market_rows, lookback_days=bounded_lookback_days, now=resolved_now)
    )
