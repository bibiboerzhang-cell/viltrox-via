...
def _market_summary_card(report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": f"本范围加载 {signals} 条竞品监测信号，其中竞品上新/动向 {launch} 条、评论机会 {comments} 条、高优先级 {high} 条。可作为新品市场方案与排期的外部输入,不代表自家销量预期。",
        ...
        cards.append(
            {
                ...
                "summary": f"竞品 {brand.upper()} 本轮出现 {count} 条、累计热度 score {score:g}(样本内势头)。作为新品市场方案/排期的外部输入信号,不代表自家销量或全市场份额。",
