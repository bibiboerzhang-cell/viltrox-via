"""Project-level message/content/terms/shipment write operations for V-KPI workflow.

行为不变搬迁:这些「项目级写操作」内聚簇(非 assignment-keyed)整簇从
workflow_evidence.py 搬来,函数体逐字未变 → 行为必然不变。workflow_evidence.py
以 from <this> import (...) re-export 兜住全部调用点。
"""
...
def _db_bool(value: Any) -> bool | int:
    # 与 workflow_evidence._db_bool 同口径(避免循环依赖,这里独立定义,行为逐字一致)。
