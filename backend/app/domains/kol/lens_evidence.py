...
归一:只认 vkpi_products 目录(+ vkpi_product_aliases 别名表);
  * sku        —— 唯一命中一个 SKU(含卡口);
  * family     —— 命中同一镜头家族(焦段+光圈+系列)但卡口未知 → 多 SKU 候选;
...
EXTRACTOR_VERSION = "lens_evidence_v1"
...
_MOUNT_TOKEN_RE = re.compile(r"^(?:fe|e|z|x|l|rf|ef|pl|m43)(?:-?mount)?$", re.IGNORECASE)
...
def _clip_body(after_brand: str) -> str:
    ...
    head = _CLIP_STOP_RE.split(after_brand, maxsplit=1)[0]
    words = head.split()
    tokens: list[str] = []
    consumed = 0
...
def extract_mentions(result: Any) -> list[Mention]:
    ...
    for field_name, text in source_texts(result):
        for match in _BRAND_RE.finditer(text):
            body = _clip_body(text[match.end():match.end() + 120])
            if not body:
                continue
            norm = normalize_mention(body)
            if not norm:
                continue
            # product_presence / brand_exposure 本身就是产品字段,线索取整段;长散文只看所在句。
            context = text if field_name in {"product_presence", "brand_exposure", "raw_viltrox_lens", "raw_viltrox_products_all"} else _sentence_for(text, match.start(), match.end() + len(body))
            item = found.get(norm)
            if item is None:
                item = Mention(text=body, norm=norm)
                found[norm] = item
            elif len(body) > len(item.text):
                item.text = body
...
class CatalogIndex:
    ...
    def _outcome(self, skus: set[str], mention: str) -> dict[str, Any]:
        ...
        if not candidates:
            return {"resolution": "unresolved", "product_sku": None, "lens_key": "", "display_name": mention, "category_main": "", "candidate_skus": []}
        if len(candidates) == 1:
            product = self.products[candidates[0]]
            return {"resolution": "sku", "product_sku": product.sku, "lens_key": product.family_key, "display_name": product.family, "category_main": product.category_main, "candidate_skus": candidates}
        families = {self.products[sku].family_key for sku in candidates}
        if len(families) == 1:
            product = self.products[candidates[0]]
            return {"resolution": "family", "product_sku": None, "lens_key": product.family_key, "display_name": product.family, "category_main": product.category_main, "candidate_skus": candidates}
        return {"resolution": "unresolved", "product_sku": None, "lens_key": "", "display_name": mention, "category_main": "", "candidate_skus": candidates}
    ...
    def resolve(self, mention: str) -> dict[str, Any]:
        ...
        # 2. 镜头规格匹配:焦段必须全等;光圈 / 系列 / 卡口有就当硬约束
        if spec["focal"]:
            ...
            if spec["mount"]:
                wanted = {spec["mount"]}
                if spec["mount"] in {"PL-mount", "L-mount"}:
                    wanted = {"PL-mount", "L-mount"}
                pool = [p for p in pool if p.mount in wanted]
            ...
            if not spec["series"] and len({p.family_key for p in pool}) > 1:
                ...
                if plain and len({p.family_key for p in plain}) == 1:
                    pool = plain
            return self._outcome({p.sku for p in pool}, mention_text)
        # 3. 非镜头型号码(DC-A1 / Z1 Pro / K60):型号 token 子串命中
        probe = key
        if len(probe) >= 3:
...
def extract_resolved(result: Any, index: CatalogIndex) -> list[dict[str, Any]]:
    ...
    for mention in extract_mentions(result):
        outcome = index.resolve(mention.text)
        group_key = (
