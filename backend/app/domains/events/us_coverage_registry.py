...
def audit_registry(payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not isinstance(event_sources, list):
        issues.append({"code": "event_sources_type", "path": "event_sources", "message": "must be an array"})
        event_sources = []
    if not isinstance(dealer_sources, list):
        issues.append({"code": "dealer_sources_type", "path": "dealer_discovery_sources", "message": "must be an array"})
        dealer_sources = []
    ...
    if data.get("full_us_coverage") is not False:
        issues.append({"code": "full_us_coverage_forbidden", "path": "full_us_coverage", "message": "must remain false"})
    if str(data.get("coverage_claim") or "") != "registered_publisher_owned_public_entries_only":
        issues.append({"code": "coverage_claim_invalid", "path": "coverage_claim", "message": "claim must remain source-bounded"})

    all_rows = [("event_sources", row, _EVENT_SOURCE_KINDS) for row in event_sources]
    all_rows += [("dealer_discovery_sources", row, _DEALER_SOURCE_KINDS) for row in dealer_sources]
    all_rows += [
    ...
    for scope, raw, allowed_kinds in all_rows:
        ...
        if not isinstance(raw, dict):
            issues.append({"code": "source_row_type", "path": path, "message": "must be an object"})
            continue
        source_id = str(raw.get("id") or "").strip()
        url = str(raw.get("canonical_url") or "").strip()
        source_kind = str(raw.get("source_kind") or "").strip()
        if not source_id or source_id in seen_ids:
            issues.append({"code": "source_id_invalid_or_duplicate", "path": f"{path}.id", "message": "stable unique id required"})
        if not _https_url(url) or url in seen_urls:
            issues.append({"code": "source_url_invalid_or_duplicate", "path": f"{path}.canonical_url", "message": "unique credential-free HTTPS URL required"})
        if source_kind not in allowed_kinds:
            issues.append({"code": "source_kind_invalid", "path": f"{path}.source_kind", "message": "source kind is not allowed for this registry scope"})
        if not str(raw.get("publisher") or "").strip():
            issues.append({"code": "publisher_missing", "path": f"{path}.publisher", "message": "publisher is required"})
        ...
        requires_explicit_review_metadata = bool(
            scope == "dealer_official_ingest_sources"
            or source_kind == "manufacturer_dealer_directory"
        )
        if requires_explicit_review_metadata and fixture_status not in _FIXTURE_STATUSES:
            issues.append(
                {
        ...
        if requires_explicit_review_metadata and str(raw.get("status") or "") != "awaiting_review":
            issues.append(
                {
        ...
        if requires_explicit_review_metadata and raw.get("enabled") is not False:
            issues.append(
                {
        ...
        if (
            requires_explicit_review_metadata
            and str(raw.get("terms_robots_status") or "") != "pending_review"
        ):
            issues.append(
                {
        ...
        if not isinstance(raw_state_codes, list):
            issues.append({"code": "state_codes_type", "path": f"{path}.state_codes", "message": "must be an array"})
        else:
            state_codes = [str(value or "").strip().upper() for value in raw_state_codes]
            invalid_state_codes = sorted({value for value in state_codes if value not in _US_JURISDICTIONS})
            ...
            if len(state_codes) != len(set(state_codes)):
                issues.append({"code": "state_code_duplicate", "path": f"{path}.state_codes", "message": "state/DC codes must be unique"})
            state_codes = sorted({value for value in state_codes if value in _US_JURISDICTIONS})
        ...
        normalized_rows.append(
            {
                **raw,
                ...
                "publisher_identity_status": str(
                    raw.get("publisher_identity_status") or "unverified"
                ),
                "claim_status": "descriptive_only",
                "scope": scope,
    ...
    dealers = [row for row in normalized_rows if row["scope"] == "dealer_discovery_sources"]
    official_ingest = [
        row
    ...
    return {
        ...
        "counts": {
            ...
            "event_source_kinds": dict(sorted(Counter(row["source_kind"] for row in events).items())),
            "dealer_source_kinds": dict(sorted(Counter(row["source_kind"] for row in dealers).items())),
