...
def _score_one(dealer: dict[str, Any], family: str | None) -> dict[str, Any]:
    ...
    score = 100.0 * (_W_GEO * geo_v + _W_CATEGORY * cat_v + _W_TIER * tier_v)
    notes: list[str] = []
    if not geo_k:
    ...
    return {
        ...
        "dealer_fit_score": round(score, 1),
        "components": {
...
def dealer_fit(sku: str, dealers: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    scored = [_score_one(d, family) for d in source_rows]
