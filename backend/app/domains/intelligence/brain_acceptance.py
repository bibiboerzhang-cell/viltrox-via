"""Pure brain-layer technical acceptance rules."""
...
def side_effects_blocked(report: dict[str, Any]) -> bool:
    return not any(
        bool(report.get(key))
...
def module_highlights(key: str, report: dict[str, Any]) -> dict[str, Any]:
...
def build_brain_layer_module_row(
    spec: dict[str, str],
    report: dict[str, Any],
    *,
    artifact_path: str = "",
    ...
    loaded = bool(artifact_path and report)
    return {
        "key": spec["key"],
        "phase": spec["phase"],
        "label": spec["label"],
        ...
        "side_effects_blocked": side_effects_blocked(report) if loaded else False,
        "highlights": module_highlights(spec["key"], report) if loaded else {},
    }
...
def build_brain_layer_acceptance_report(
