...
def _expected_worker_count() -> int:
    ...
    return int(expected_raw) if expected_raw.isdigit() and int(expected_raw) > 0 else 1


def _empty_fleet_health(expected: int) -> dict[str, Any]:
    return {
        "online": False,
        "online_count": 0,
...
def _worker_health(row: Any, now: datetime) -> dict[str, Any]:
...
def redis_worker_fleet_health(app_git_sha: str = "") -> dict[str, Any]:
    expected = _expected_worker_count()
    if not table_exists("vkpi_worker_heartbeat"):
        return _empty_fleet_health(expected)
    ...
    workers = [_worker_health(row, now) for row in rows]
    return _fleet_projection(workers, expected, app_git_sha)
